...
We will see how to create ndarrays by providing Python lists to the NumPy `np.array()` function. This can create some confusion for beginners, but it is important to remember that `np.array()` is *NOT* a class, it is just a function that returns an ndarray. Let’s start by creating 1-Dimensional (1D) ndarrays.
...
# Lets print tha ndarray we just created using the print() command
print('x = ', x)
...
# Here the zeros() is a built-in function
...
It returns a tuple representing the array dimensions. Refer more details [here(opens in a new tab)](https://numpy.org/doc/stable/reference/generated/numpy.ndarray.shape.html#numpy-ndarray-shape).
...
As we can see the shape attribute tells us that `x` now only has 2 elements, and even though `x`now holds strings, the `type()` function tells us that `x` is still an ndarray as before. In this case however, the `.dtype` attribute tells us that the elements in `x` are stored in memory as *Unicode strings of 5 characters*.

Its is important to remember that one big difference between Python lists and ndarrays, is that unlike Python lists, all the elements of an ndarray must be of the same type. So, while we can create Python lists with both integers and strings, we can’t mix types in ndarrays. If you provide the `np.array()` function with a Python list that has both integers and strings, NumPy will interpret all elements as strings. We can see this in the next example:
...
We can see that when we create an ndarray with only floats, NumPy stores the elements in memory as *64-bit floating point number (float64)*. However, notice that when we create an ndarray with both floats and integers, as we did with the `z` ndarray above, NumPy assigns its elements as *float64* dtype as well. This is called *upcasting*. Since all the elements of an ndarray must be of the same type, in this case NumPy upcasts the integer in `z` to floats in order to avoid losing precision in numerical computations.

## Using a 1-D Array of Float, and specifying the dtype of each element

Even though NumPy automatically selects the dtype of the ndarray, NumPy also allows you to specify the particular dtype you want to assign to the elements of the ndarray. You can specify the dtype when you create the ndarray using the keyword `dtype` in the `np.array()` function. Lets see an example:
...
print('x = ', x)
print('The elements in x are of the type: ', x.dtype)
...
Another useful attribute is [NumPy.size(opens in a new tab)](https://numpy.org/doc/stable/reference/generated/numpy.ndarray.size.html), which returns the number of elements in the array. Let us now look at how we can create a rank 2 ndarray from a nested Python list.
...
# We save it into the current directory as
np.save('my_array', x)
```

The above saves `x` ndarray into a file named `my_array.npy`. You can load the saved ndarray into a variable by using the `load()` function.
...
When loading an array from a file, make sure you include the name of the file together with extension `.npy`, otherwise you will get an error.