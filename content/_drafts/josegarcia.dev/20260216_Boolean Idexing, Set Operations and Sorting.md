...
# We use set operations to compare x an y:
print()
...
The method above sorts an array in place. All arguments are optional, see the details [here(opens in a new tab)](https://numpy.org/doc/stable/reference/generated/numpy.ndarray.sort.html).
...
- If `axis = 0` is specified for a given 2-D array —for one column at a time, the function will sort all rows, without disturbing other elements. In the final output, you will see that each column has bee sorted individually.
...
Refer [here(opens in a new tab)](https://numpy.org/doc/stable/reference/generated/numpy.sort.html) for details about the optional arguments.
...
Try out the advanced examples of indexing available at [NumPy.org](http://NumPy.org)[ - Indexing](https://numpy.org/devdocs/user/basics.indexing.html)